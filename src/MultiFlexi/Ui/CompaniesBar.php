    ...
    public function __construct(array $properties = [])
    {
        ...
        foreach ($companies as $companyData) {
            ...
            foreach ($companyApps as $companyApp) {
                ...
                if ($successJobs > 0) {
                    $jobCounts->addItem(new ATag('jobs.php?app_id='.$appId.'&company_id='.$companyId.'&status=success', (string) $successJobs, ['class' => 'badge rounded-pill text-bg-success']));
                }

                if ($failedJobs > 0) {
                    $jobCounts->addItem(new ATag('jobs.php?app_id='.$appId.'&company_id='.$companyId.'&status=failed', (string) $failedJobs, ['class' => 'badge rounded-pill text-bg-danger']));
                }

                if ($waitingJobs > 0) {
                    $jobCounts->addItem(new ATag('jobs.php?app_id='.$appId.'&company_id='.$companyId.'&status=waiting', (string) $waitingJobs, ['class' => 'badge rounded-pill text-bg-warning']));
                }
