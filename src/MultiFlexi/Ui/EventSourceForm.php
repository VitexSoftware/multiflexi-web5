...
class EventSourceForm extends SecureForm
{
    ...
    {
        $row1 = new \Ease\TWB5\Row();
        $row1->addColumn(6, [
            new \Ease\TWB5\FormGroup(_('Source Name'), new \Ease\Html\InputTextTag('name', $eventSource->getDataValue('name'))),
            new \Ease\TWB5\FormGroup(_('Adapter Type'), new \Ease\Html\InputTextTag('adapter_type', $eventSource->getDataValue('adapter_type'), [], _('e.g. abraflexi-webhook-acceptor'))),
        ]);
        $row1->addColumn(6, [
            new \Ease\TWB5\FormGroup(_('Poll Interval (seconds)'), new \Ease\Html\InputNumberTag('poll_interval', (string) ($eventSource->getDataValue('poll_interval') ?: '60'))),
        ...
        $row2 = new \Ease\TWB5\Row();
        $row2->addColumn(4, [
            new \Ease\TWB5\FormGroup(_('DB Driver'), new \Ease\Html\SelectTag('db_connection', [
                'mysql' => 'MySQL / MariaDB',
                'pgsql' => 'PostgreSQL',
                'sqlite' => 'SQLite',
        ...
        $row2->addColumn(4, [
            new \Ease\TWB5\FormGroup(_('DB Host'), new \Ease\Html\InputTextTag('db_host', $eventSource->getDataValue('db_host') ?: 'localhost')),
        ]);
        $row2->addColumn(4, [
            new \Ease\TWB5\FormGroup(_('DB Port'), new \Ease\Html\InputTextTag('db_port', $eventSource->getDataValue('db_port') ?: '3306')),
        ]);

        $row3 = new \Ease\TWB5\Row();
        $row3->addColumn(4, new \Ease\TWB5\FormGroup(_('Database Name'), new \Ease\Html\InputTextTag('db_database', $eventSource->getDataValue('db_database'))));
        $row3->addColumn(4, new \Ease\TWB5\FormGroup(_('DB Username'), new \Ease\Html\InputTextTag('db_username', $eventSource->getDataValue('db_username'))));
        $row3->addColumn(4, new \Ease\TWB5\FormGroup(_('DB Password'), new \Ease\Html\InputPasswordTag('db_password', $eventSource->getDataValue('db_password'))));
