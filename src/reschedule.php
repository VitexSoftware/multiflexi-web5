...
if ($jobber->getDataValue('begin')) {
    WebPage::singleton()->container->addItem(new \Ease\TWB5\Alert('warning', _('This job has already been executed and cannot be re-scheduled.')));
    WebPage::singleton()->container->addItem(new \Ease\TWB5\LinkButton('job.php?id='.$jobID, _('Back to Job'), 'info'));
...
if ($jobber->isScheduled()) {
    WebPage::singleton()->container->addItem(new \Ease\TWB5\Alert('info', _('This job is already scheduled in the queue.')));
...
if (WebPage::isPosted() || $when === 'now') {
    ...
    try {
        ...
        WebPage::singleton()->container->addItem(new \Ease\TWB5\Alert('success', [
        ...
        WebPage::singleton()->container->addItem(new \Ease\TWB5\LinkButton('job.php?id='.$jobID, _('Back to Job'), 'info btn-lg'));
        WebPage::singleton()->container->addItem(new \Ease\TWB5\LinkButton('queue.php', _('View Queue'), 'primary btn-lg'));
    } catch (\Exception $e) {
        WebPage::singleton()->addStatusMessage(
        ...
        WebPage::singleton()->container->addItem(new \Ease\TWB5\Alert('danger', [
    ...
    $panel = new \Ease\TWB5\Panel(_('Re-schedule Orphaned Job'), 'warning');

    $panel->addItem(new \Ease\TWB5\Alert('warning', [
        '⚠️ ',
